    ...
    {isDrawerOpen && <>
      ...
      <div className="fixed top-0 right-0 h-full w-80 bg-gradient-to-b from-gray-50 to-white shadow-xl z-50 lg:hidden transform transition-transform duration-300 ease-in-out">
        <div className="flex flex-col h-full">
          ...
          {isSignedIn && <div className="px-4 py-4 border-t border-gray-200 bg-gray-50">
            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-between px-4 py-3 bg-white border border-red-100 text-red-600 rounded-xl shadow-sm active:bg-red-50 transition-all font-bold text-sm"
            >
              <span>Sign Out</span>
              <LogOutIcon size={18} />
            </button>
            <div className="text-center mt-3">
              <span className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">Digital Qatalyst</span>
            </div>
          </div>}
        </div>
      </div>
