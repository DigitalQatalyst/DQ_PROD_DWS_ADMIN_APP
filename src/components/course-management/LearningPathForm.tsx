...
import { DEPARTMENTS, COURSE_CATEGORIES, LMS_ITEM_PROVIDERS, SFIA_LEVEL_CODES, AUDIENCE_OPTIONS } from '../../constants/courseConstants';

export const LearningPathForm: React.FC = () => {
  ...
  const loadLearningPath = async (pathId: string) => {
    try {
      setLoading(true);
      ...
      setToast({ type: 'error', message: error.message || 'Failed to load learning path' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Use slug or generate a temporary one for new learning paths
    const courseSlug = formData.slug || formData.title.toLowerCase().replace(/\s+/g, '-') || 'temp';

    if (!courseSlug || courseSlug === 'temp') {
      setToast({ type: 'error', message: 'Please enter a title or slug first' });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToast({ type: 'success', message: `Learning path ${isEditing ? 'updated' : 'created'} successfully` });
      setTimeout(() => {
        navigate('/course-management?tab=learning-paths');
      }, 1000);
    } catch (error: any) {
      console.error('Error saving learning path:', error);
      setToast({ type: 'error', message: error.message || 'Failed to save learning path' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="px-4 sm:px-6 pt-4 pb-20 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <button
            onClick={() => navigate('/course-management?tab=learning-paths')}
