-- LMS (Learning Management System) Schema - Simplified for MS Entra ID
-- This version works without auth_user_profiles table
...
-- RLS Policies - Simplified for MS Entra ID
-- Enable RLS
...
-- Read policies: Allow public read access to all LMS content
...
-- Write policies: Allow all authenticated users (restrict at application level)
-- NOTE: Since you're using MS Entra ID, authentication is handled at the app level
-- You can restrict access using your application's RBAC system
CREATE POLICY "Allow authenticated users to manage learning paths" ON public.lms_learning_paths
    FOR ALL USING (true); -- Adjust based on your needs
